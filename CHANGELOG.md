...
#### 1. 路线规划 - 真实道路路径渲染
- 行程详情页地图改用高德地图 API 2.0（原 1.4.15 已废弃）
- 步行路段调用 `AMap.Walking` 插件，按真实道路折线绘制
- 自行车/电瓶车路段调用 `AMap.Riding` 插件，兼容 2.0 结果结构（`rideRoutes`）

#### 2. 路线规划 - 切换交通方式自动 TSP 重排
- 切换步行/自行车/电瓶车时，系统自动用新交通方式的时间代价重跑 TSP
- 景点列表按最优通行顺序重新排列，再用对应插件绘制真实道路路径

#### 3. 高德地图密钥配置优化
- 支持 `VITE_AMAP_SECURITY_KEY` 安全密钥（高德 Web JS API 2.0 要求）
- 启动前自动注入 `window._AMapSecurityConfig`

### 🐛 修复
- 修复 `AMap.Riding` 回调结果中 `rides` 字段（而非 `steps`）导致坐标提取失败、路径始终为直线的问题
- 修复步行路径在部分城市降级为直线的问题（`steps[0].path` 字段提取逻辑修正）
- 修复高德地图因版本 1.4.15 废弃导致加载失败的问题
