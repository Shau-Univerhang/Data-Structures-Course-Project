  ...
  <div class="internal-nav-page">
    <header class="nav-header">
      ...
      <div class="header-center">
        ...
        <span class="header-sub">路线规划 · 场所查询</span>
      </div>
      <div class="header-badge">
        {{ mapData.spot?.type === "campus" ? "校园" : "景区" }}
      </div>
    </header>
      ...
      <aside class="control-panel">
        ...
        <section class="panel-card quick-card">
            ...
            <div class="selected-node-title-row">
              <div>
                ...
                <div class="selected-node-meta">
                  {{ typeLabel(selectedMapNode.type) }}
                </div>
              </div>
              <button class="mini-btn ghost" @click="selectedMapNode = null">
                关闭
              </button>
            </div>
            <div class="action-grid">
              <button class="mini-btn" @click="setAsStart(selectedMapNode)">
                设为起点
              </button>
              <button class="mini-btn" @click="setAsEnd(selectedMapNode)">
                设为终点
              </button>
              <button class="mini-btn" @click="addAsWaypoint(selectedMapNode)">
                加入途经点
              </button>
              <button
                class="mini-btn"
                @click="openNearbyForNode(selectedMapNode)"
              >
                查看附近
              </button>
            </div>
          ...
          <div v-else class="hint-card">
            点击左侧搜索结果或地图节点，可以设为起点、加入路线或查看附近设施。
          </div>
        </section>
        ...
        <section class="panel-card tabs-card">
          <div class="top-tabs">
            <button
              :class="['top-tab', { active: activePanel === 'route' }]"
              @click="activePanel = 'route'"
            >
              路线规划
            </button>
            <button
              :class="['top-tab', { active: activePanel === 'nearby' }]"
              @click="activePanel = 'nearby'"
            >
              场所查询
            </button>
          </div>
        ...
        <template v-if="activePanel === 'route'">
          ...
          <section class="panel-card">
            ...
            <div class="transport-tabs transport-tabs-icons">
              <button
                ...
                :class="[
                  'transport-tab',
                  'transport-tab-icon',
                  { active: transportMode === mode.value },
                ]"
                @click="transportMode = mode.value"
              >
                <span class="transport-icon">{{
                  transportIcon(mode.value)
                }}</span>
            ...
            <div class="transport-tabs small-gap top-gap">
              <button
                :class="[
                  'transport-tab secondary',
                  { active: strategy === 'shortest_time' },
                ]"
                @click="strategy = 'shortest_time'"
              >
                最短时间
              </button>
              <button
                :class="[
                  'transport-tab secondary',
                  { active: strategy === 'shortest_distance' },
                ]"
                @click="strategy = 'shortest_distance'"
              >
                最短距离
              </button>
            </div>
          ...
          <section class="panel-card">
            ...
            <div v-if="waypointNodes.length" class="selected-list">
              ...
              >
                ...
                <button class="remove-btn" @click="removeWaypoint(target.id)">
                  ×
                </button>
              </div>
            </div>
          ...
          <section
            v-if="routeResult && !routeResult.error"
            class="panel-card result-card"
          >
            ...
            <div class="result-grid">
              <div class="metric-card">
                ...
                <strong
                  >{{ (routeResult.distance / 1000).toFixed(2) }} km</strong
                >
              </div>
              <div class="metric-card">
            ...
            <div v-if="transportSegments.length" class="chips-wrap">
              ...
              <span
                v-for="segment in transportSegments"
                :key="segment.key"
                class="mode-chip"
        ...
        <template v-else-if="activePanel === 'nearby'">
          ...
          <section class="panel-card">
            ...
            <div class="chips-wrap">
              <button
                :class="['filter-chip', { active: nearbyCategory === 'all' }]"
                @click="nearbyCategory = 'all'"
              >
                全部
              </button>
              <button
                ...
                :class="[
                  'filter-chip',
                  { active: nearbyCategory === category },
                ]"
            ...
            <div class="transport-tabs small-gap top-gap">
              <button
                :class="[
                  'transport-tab secondary',
                  { active: nearbyStrategy === 'shortest_time' },
                ]"
                @click="nearbyStrategy = 'shortest_time'"
              >
                按时间排序
              </button>
              <button
                :class="[
                  'transport-tab secondary',
                  { active: nearbyStrategy === 'shortest_distance' },
                ]"
                @click="nearbyStrategy = 'shortest_distance'"
              >
                按距离排序
              </button>
            </div>
            ...
            <button
              ...
              :disabled="nearbyLoading || !nearbyOriginNodeId"
              @click="fetchNearbyFacilities"
            >
          ...
          <section class="panel-card">
            ...
            <div v-if="nearbyResults.length" class="selected-list results-list">
              ...
              >
                <button
                  class="nearby-preview"
                  @click="previewNearbyResult(item)"
                >
                  <div class="nearby-main-row">
                    <strong>{{ item.name }}</strong>
                    <span class="candidate-type-badge">{{
                      facilityLabel(item.type)
                    }}</span>
                  </div>
                  ...
                  <div class="nearby-path-row">
                    ...
                    <span v-if="item.segment_transport_modes?.length">{{
                      item.segment_transport_modes
                        .map((mode) => transportIcon(mode))
                        .join(" ")
                    }}</span>
                  </div>
                  <div v-if="item.description" class="nearby-desc">
                    {{ item.description }}
                  </div>
                </button>
            ...
            <div v-else class="hint-card">
              查询后会按图距离排序展示超市、卫生间、食堂、医务室等设施。
            </div>
          </section>
        </template>

        <section
          v-if="loadError || routeError || nearbyError || emptyStateText"
          class="status-stack"
        >
          <div v-if="loadError" class="status-card error-card">
            {{ loadError }}
          </div>
          <div v-else-if="routeError" class="status-card error-card">
            {{ routeError }}
          </div>
          <div v-else-if="nearbyError" class="status-card error-card">
            {{ nearbyError }}
          </div>
          <div v-else-if="emptyStateText" class="status-card empty-card">
            {{ emptyStateText }}
          </div>
        </section>
      </aside>
...
const AMAP_KEY =
  import.meta.env.VITE_AMAP_KEY || "3350803110b3a0e1d1f84585f7976cf4";
const AMAP_SECURITY_KEY =
  import.meta.env.VITE_AMAP_SECURITY_KEY || "a7e5b0c229ff2ac54dd6fc4fc9fc489d";
...
const NUMBERED_BUILDING_REGEX = /^\d+号楼$/;

function isExcludedNode(node) {
  if (!node || !node.name) return false;
  // 过滤停车点（名称含"停车点"）
  if (node.name.includes("停车点")) return true;
  // 过滤运动设施（名称含"运动设施"或"健身中心"）
  if (node.name.includes("运动设施") || node.name.includes("健身中心"))
    return true;
  // 过滤几号楼（如 7号楼、11号楼）
  if (NUMBERED_BUILDING_REGEX.test(node.name)) return true;
  return false;
}

const anchorNodes = computed(() =>
  (mapData.value.nodes || []).filter(
    (node) =>
...
const nodeMap = computed(() =>
  Object.fromEntries(
    (mapData.value.nodes || []).map((node) => [node.id, node]),
  ),
);
...
const startNodeName = computed(
  () => nodeMap.value[startNodeId.value]?.name || "未选择",
);
const endNodeName = computed(
  () => nodeMap.value[endNodeId.value]?.name || "未选择",
);
const waypointNodes = computed(() =>
  waypointIds.value.map((id) => nodeMap.value[id]).filter(Boolean),
);
const plannerModes = computed(() =>
  displayModes.value.filter((mode) => ["walk", "bike"].includes(mode.value)),
);
...
const hasRouteSelection = computed(() =>
  Boolean(
    startNodeId.value ||
    endNodeId.value ||
    waypointIds.value.length ||
    routeResult.value,
  ),
);
const strategyLabel = computed(() =>
  strategy.value === "shortest_time" ? "最短时间" : "最短距离",
);
const facilityCategories = computed(
  () => mapData.value.facility_categories || [],
);
const emptyStateText = computed(() => {
  if (loading.value) return "";
  if (!mapData.value.nodes.length) return "当前景点暂无内部路网数据。";
  if (nodeSearchKeyword.value.trim() && !filteredAnchorNodes.value.length)
    return "没有匹配的建筑或设施。";
  return "";
});
const nearbyOriginName = computed(
  () => nodeMap.value[nearbyOriginNodeId.value]?.name || startNodeName.value,
);
...
function typeLabel(type) {
  return (
    { building: "建筑", facility: "设施", entrance: "入口", crossing: "路口" }[
      type
    ] || type
  );
}
...
function setAsStart(node) {
  if (!node || isExcludedNode(node)) return;
...
function clearRouteSelections() {
  ...
  nearbyResults.value = [];
  selectedMapNode.value = startNodeId.value
    ? nodeMap.value[startNodeId.value] || null
    : null;
  resetRouteArtifacts();
}
...
async function loadMapData() {
  ...
  try {
    const res = await fetch(
      `${API_BASE}/api/route/internal-map/${spotId.value}`,
    );
    ...
    transportMode.value = plannerModes.value.some(
      (mode) => mode.value === transportMode.value,
    )
      ? transportMode.value
...
async function fetchNearbyFacilities() {
  ...
  try {
    ...
    if (nearbyCategory.value && nearbyCategory.value !== "all")
      params.set("category", nearbyCategory.value);
    if (nearbyKeyword.value.trim())
      params.set("keyword", nearbyKeyword.value.trim());
    params.set("strategy", nearbyStrategy.value);

    const res = await fetch(
      `${API_BASE}/api/route/nearby-facilities?${params.toString()}`,
    );
    if (!res.ok) throw new Error(`接口返回 ${res.status}`);
    const data = await res.json();
    if (data.error) throw new Error(data.error);
    ...
    mapData.value.facility_categories =
      data.facility_categories || mapData.value.facility_categories;
...
async function initMap() {
  ...
  map = new AMap.Map("internal-nav-map", {
    zoom: 16,
    center: mapPoint(
      mapData.value.spot?.location_lng || 116.35255,
      mapData.value.spot?.location_lat || 39.9612,
    ),
    resizeEnable: true,
    viewMode: "2D",
  });
...
function drawRoadNetwork() {
  ...
  for (const edge of mapData.value.edges) {
    ...
    if (!from || !to) continue;
    const key = [
      Math.min(edge.from_node_id, edge.to_node_id),
      Math.max(edge.from_node_id, edge.to_node_id),
      edge.road_type,
    ].join("-");
